...
export default ({ managementPractices, location, cropsData, selectedSeedVariety, selectedSowingMethod }: { managementPractices: object, location: object, cropsData: object, selectedSeedVariety: string, selectedSowingMethod: string }) => {
  ...
  return (
    <ScrollArea className="h-[300px]">
      <div className="w-full">
        <div>
          {
            managementPractices?.list?.Land_Preparation?.map((item: any, index: number) => {
              return (
          ...
          <div className="w-[85%] bg-white rounded-md shadow-3xl font-tahoma flex flex-col justify-center p-2">
            ...
            <div className='w-44 mt-2 self-center'>
              <Btn
                ...
                textColor="white"
                onClick={() => {
                  if (userId.id && cropsData.crop && suitableCrops[0].season && location.lat && location.lng && date && selectedLand?.selectedLand?.id && selectedSeedVariety && suitableCrops[0].s_range && suitableCrops[0].e_range && selectedSowingMethod) {
                    postCropRecord(userId.id, cropsData.crop, suitableCrops[0].season, location.lat, location.lng, date, selectedLand?.selectedLand?.id, selectedSeedVariety, suitableCrops[0].s_range, suitableCrops[0].e_range, selectedSowingMethod).then((res) => {
                      console.log('res: ', res)
